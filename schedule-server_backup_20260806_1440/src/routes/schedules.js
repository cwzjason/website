...
module.exports = function (db) {
  ...
  router.get('/', (req, res) => {
    ...
    // 注意：SQLite 中 '' < datetime(...) 返回 1（TRUE），必须排除空 end_time
    try {
      db.prepare(
        "UPDATE schedules SET status = '已完成', updated_at = datetime('now','localtime') WHERE status = '待办' AND end_time != '' AND end_time < datetime('now','localtime') AND openid = ?"
      ).run(openid);
    } catch (e) {
      // 静默忽略
    }
